...
author: "Elaborado por: Diego Gomez"
date: "Junio, 2024"
output:
  pdf_document :
    toc: true
...
#include_graphics("./logo_umsa2.png")

#\newpage
```

=======
>>>>>>> 0595552014b9cc172ddac92e999e5e933bedc64f
\newpage
# Introducción
...
Al respecto, el Grupo Financiero Patito S.A. observando el mercado potencial, determinó incursionar en la comercialización de seguros a través del Banco Patito S.A. en la modalidad de Seguros de Personas y Generales.

# Objetivos

## Objetivo General

- Identificar los patrones de comportamientos mediante el análisis de k-medias de los clientes con créditos de seguros del Banco Patito.
...
# Motivación
Banco Patito S.A., al contar con una cantidad importante de clientes con canales de alto impacto, pretende conocer el comportamiento de los clientes para poder incrementar los ingresos por la comercialización de seguros. 

Se advierte que el volumen de la cartera del Banco Patito S.A, créditos PYME, microcréditos, vivienda y consumo dirigido a personas naturales con cobertura del seguro, tuvo crecimiento en las gestiones 2017-2020 y una reducción al cierre de 2021 como resultado de la contracción económica debido a la pandemia, ya que en esta etapa se otorgó el diferimiento y reprogramación de créditos, pero para la gestión 2022 se observa una recuperación de la cartera en riesgo con un crecimiento menor en valor absoluto de la gestión 2020.
...
En el entorno actual, la gestión efectiva de metas e indicadores es fundamental para el éxito en la industria financiera y aseguradora. Desarrollar un dashboard específico para la entidad financiera permitirá visualizar y analizar de manera eficiente los datos clave relacionados con la comercialización de seguros. Esto facilitará la toma de decisiones informadas y la identificación de áreas de mejora.
...
# Marco Teórico
El seguimiento de ventas y la concentración de ingresos son elementos cruciales para el éxito de cualquier organización comercial. Este marco teórico explora los fundamentos esenciales de estos conceptos y cómo su comprensión y aplicación pueden contribuir a una gestión empresarial efectiva.
...
La base de datos actual proviene de las transacciones de un mes del Banco Patito S.A. respecto a ingresos no financieros (seguros). En la base de datos se cuenta con *41 variables* y 200K registros de evaluación respecto a datos del cliente , del seguro y operación crediticia. Se detalla a continuación las variables más relevantes para la construcción de los indicadores:
...
### Tasa de ingresos por prima promedio por regional
```{r echo=FALSE, message=FALSE, warning=FALSE}
data2 <- data.frame(
  Campo = c("•	Nombre del Indicador"
            , "•	Sigla"
            , "•	Objetivo:"
            ,"•	Método de medición: "
            ,"•	Unidad de Medida: "
  ...
  Concepto = c("Tasa de ingresos por prima promedio por regional"
               , "INGREG"
               , "Comparar el volumen de ingresos por departamento respecto a la meta comercial de seguros"
               ," Realizar la agrupación de ingresos de prima por departamento y comparar respecto a la meta comercial"
               ," En moneda nacional Bs."
               ," El volumen de ingresos por prima refiere a lo recaudado por venta de seguros en el banco."
               ," vol prima por depto / meta de vol prima"
       ," Prima y departamento"
," Minimo 1"
)
)

kable(data2,format = "pipe", padding = 2)


```
### Pareto de primas de seguros 
```{r echo=FALSE, message=FALSE, warning=FALSE}
data3 <- data.frame(
  Campo = c("•	Nombre del Indicador"
            , "•	Sigla"
  ...
  Concepto = c("Pareto de primas de seguros"
               , "INGPROMREG"
               , "Conocer si el 80% de los ingresos no proviene de los principales 2000 clientes de créditos."
               ," Se categoriza a los 2000 principales clientes de créditos para conocer su concentración respecto a la cartera de créditos."
               ," % "
               ," La concentración de riesgo es relevante para diversificar la cartera de clientes y no tener dependencia."
               ," % de concentración / 80%"
       ," "
," Menor 1"
)
...
kable(data3,format = "pipe", padding = 2)

```

### Porcentaje de participación de créditos por regiones
```{r echo=FALSE, message=FALSE, warning=FALSE}
data4 <- data.frame(
  Campo = c("•	Nombre del Indicador"
            , "•	Sigla"
  ...
  Concepto = c("% porcentaje de participación de créditos por regiones"
               , "%REGCRED"
               , "Identificar la concentración de ingresos por seguros, según regionales (altiplano, valle y oriente)"
               ," Agrupar los ingresos de seguros por regional para identificar que no supere el margen establecido"
               ," % "
               ," La concentración de ingresos por regional define la proporción de recursos que requiere cada regional así como su concentración."
               ," % ingresos por regional/% concentración esperada por regional"
       ," "
," Max 120% y min 80%"
)
)

kable(data4,format = "pipe", padding = 2)

```
### Tasa promedio de valor asegurado según cartera de créditos
```{r echo=FALSE, message=FALSE, warning=FALSE}
data5 <- data.frame(
  Campo = c("•	Nombre del Indicador"
            , "•	Sigla"
  ...
  Concepto = c("Tasa promedio de valor asegurado según cartera de créditos."
               , "PROMVACRED"
               , "Identificar el valor promedio de créditos que se tiene por tipo de crédito para identificar mayor concentración de seguros."
               ," Agrupar por tipo de crédito y calcular el promedio de cada uno para categorizar los créditos."
               ," En moneda nacional Bs. "
               ," El valor asegurado por crédito define que tipo de crédito es el que representa menor cantidad de cliente pero mayor volumen de ingresos."
               ," Máx (valor asegurado según tipo de crédito) "
       ," "
," Crédito hipotecario"
)
)

kable(data5,format = "pipe", padding = 2)

```
###  Cuartiles de cantidad de clientes por saldo de crédito
```{r echo=FALSE, message=FALSE, warning=FALSE}
data6 <- data.frame(
  Campo = c("•	Nombre del Indicador"
            , "•	Sigla"
  ...
  Concepto = c("Cuartiles de cantidad de clientes por saldo de crédito"
               , "DISTCANCLIEN"
               , "Conocer si el 75% de los créditos en cantidad tiene una cantidad similar en voluemn de créditos"
               ," Calcular los cuartiles de cantidad de clientes para comparar que el tercer cuartil no sea en volumen de créditos mayor al 75%."
               ," %"
               ," La concentración de riesgo es relevante para diversificar la cartera de créditos."
               ," % de concentración / 75% "
       ," "
," Menor 1"
)
...
kable(data6,format = "pipe", padding = 2)

```
### Porcentaje de uso de moneda nacional por volumen de crédito y cantidad
```{r echo=FALSE, message=FALSE, warning=FALSE}
data7 <- data.frame(
  Campo = c("•	Nombre del Indicador"
            , "•	Sigla"
  ...
  Concepto = c("Porcentaje de uso de moneda nacional por volumen de crédito y cantidad"
               , "%MONCRED"
               , "Conocer la concentración en moneda extranjera para identificar riesgos de tipo de cambio"
               ," Calcular el porcentaje de créditos en moneda nacional y extranjera para verificar su concentración."
               ," %"
               ," El riesgo de tipo de cambio ante la fluctuación de falta de dolares"
               ," vol. Creditos en moneda extranjera / vol créditos total"
       ," "
," Menor al 5%"
)
)

kable(data7,format = "pipe", padding = 2)

```


### Volumen de ingresos por tipo de seguro respecto a la meta
```{r echo=FALSE, message=FALSE, warning=FALSE}
data8 <- data.frame(
  Campo = c("•	Nombre del Indicador"
            , "•	Sigla"
  ...
  Concepto = c("Volumen de ingresos por tipo de seguro respecto a la meta "
               , "INGSEG"
               , " Comparar el volumen de ingresos por tipo de seguro respecto a la meta comercial de colocación por departamento "
               ," Realizar la agrupación de ingresos de prima por tipo de seguro y comparar respecto a la meta comercial."
               ," En moneda nacional Bs."
               ," El volumen de ingresos por prima refiere a lo recaudado por venta de seguros en el banco por departamento."
               ," vol prima por tipo d seguro / meta de vol prima"
       ," Prima y departamento "
," Minimo 1"
)
)

kable(data8,format = "pipe", padding = 2)

```
###  Perfil de edades según segmento requerido
```{r echo=FALSE, message=FALSE, warning=FALSE}
data9 <- data.frame(
  Campo = c("•	Nombre del Indicador"
            , "•	Sigla"
  ...
  Concepto = c("Perfil de edades según segmento requerido"
               , "PERFEDAD"
               , " Identificar el tipo de cliente que es cliente del banco respecto a tipo de crédito"
               ," Calcular la frecuencia por rango de edades para identificar la concentración de cliente por tipo de crédito."
               ," Edad de clientes por crédito"
               ," La edad define el tipo de cliente que tiene la entidad financiera para dirigir y definir estrategia comercial."
               ," Edad promedio por tipo de crédito"
       ," "
," Entre 30 y 45 años "
)
)

kable(data9,format = "pipe", padding = 2)

```
### Tasa de interes respecto al monto desembolsado asegurado
```{r echo=FALSE, message=FALSE, warning=FALSE}
data10 <- data.frame(
  Campo = c("•	Nombre del Indicador"
            , "•	Sigla"
  ...
  Concepto = c("Tasa de interes respecto al monto desembolsado asegurado"
               , "TASAMONTO"
               , "Comparar las diferentes tasas de interes en relación al monto que se desombolsa al cliente"
               ," Realizar una vista de diagrama de dispersión entre tasa de interes y monto desembolsado"
               ," Pocentaje y en Milllones de bolivianos"
               ," El monto desembolsado es la cantidad que recibe un cliente a cambio de una tasa de interes "
               ," "
       ," Monto desembolsado, tasa prima interes "
," "
)
)

kable(data10,format = "pipe", padding = 2)

```

### Tasa promedio de prima por tipo de operacion crediticia
```{r echo=FALSE, message=FALSE, warning=FALSE}
data11 <- data.frame(
  Campo = c("•	Nombre del Indicador"
            , "•	Sigla"
  ...
  Concepto = c("Tasa promedio de prima por tipo de operacion crediticia"
               , "AVGPRIMAOPE"
               , "Comparar la tasa promedio de prima en los diferentes tipos de operación crediticia "
               ," Calcular el promedio de prima agrupando por tipo de operación crediticia"
               ," %"
               ," La operación crediticia se refiere a la razón por la cual se esta haciendo el desembolso a un cliente. "
               ," Suma de primas de todos los clientes/ numero de desembolsos"
       ," Operación Crediticia, Prima"
," "
)
)

kable(data11,format = "pipe", padding = 2)

```
### Actividad economica de los asegurados por edad
```{r echo=FALSE, message=FALSE, warning=FALSE}
data12 <- data.frame(
  Campo = c("•	Nombre del Indicador"
            , "•	Sigla"
  ...
  Concepto = c("Actividad economica de los asegurados por edad"
               , "ACTECOEDAD"
               , " Identificar las actividades economicas de los clientes y sus diferencias en cuanto a primas y otros"
               ," Calcular indicadores como la media de edad, mediana de la prima y mediana del valor asegurado por actividad economica"
               ," Varios"
               ," La actividad economica se refiere a la acción que realizar los diferentes clientes con el fin de generar ingresos monetarios "
               ," Media, mediana"
       ," Actividad economica, edad, prima, valor asegurado"
," "
)
)

kable(data12,format = "pipe", padding = 2)

```

### Tasa de participación en seguros por región y tipo de seguro
```{r echo=FALSE, message=FALSE, warning=FALSE}
data13 <- data.frame(
  Campo = c("•	Nombre del Indicador"
            , "•	Sigla"
  ...
  Concepto = c("Tasa de participación en seguros por región y tipo de seguro "
               , " TASAREGIONTIPOSEG "
               , " Comparar la proporción de clientes en los diferentes tipo de seguro y región "
               ," Calcular la proporción de clientes que existen por región agrupado por tipo de seguro "
               ," %"
               ," La región nos indica el área geografica de Bolivia dividida en altiplano, oriente, valle "
               ," numero de clientes en cada región dividido entre el total de clientes en cartera, agrupado por tipo de seguro."
       ," Tipo de seguro, región"
," "
)
)

kable(data13,format = "pipe", padding = 2)
...
## Visualización sobre la información

Visualización por indicador: 

1. Tasa de ingresos por prima promedio por regional: Gráfico de barras
2. Ingresos total por regional: Graficvo de barras comparación de ingreso real y metas
3. % porcentaje de participación de créditos por regiones: Grafico de barras horizontal
4. Tasa promedio de valor asegurado según cartera de créditos: Graífico de puntos 
5. Cuartiles de valor asegurado por tipo de crédito: boxplot 
6. Porcentaje de uso de moneda nacional por volumen de crédito y cantidad: Gráfico de torta 
7. Volumen de ingresos por tipo de seguro respecto a la meta: 
8. Perfil de edades según segmento requerido: gráfico de densidad
9. Tasa de interes respecto al monto desembolsado asegurado: Gráfico de dispersión
10. Tasa promedio de prima por tipo de operacion crediticia: Gráfico de barras
11. Actividad economica de los asegurados por edad: Tabla con indicadores
12. Tasa de participación en seguros por región y tipo de seguro: Gráfico de barras apiladas.



## Armado de la Plataforma
Una vez obtenido los indicadores de la base de datos, los mismos de muestran en un dashboard resumen donde la construcción se la hizo en R, mediante la libreia **flexdashboard**. donde por cada ventana se muestran de 2 a 3 indicadores que muesran el estado actual de la empresa, en cuestión de metas, indicadores que muestran los objetivos economicos como sociales de la empresa. 
...
La base de datos de clientes de créditos y seguros cuenta con 211494 observaciones y 42 variables por lo que se realizó la preparación con la transformación de variables inicialmente para la creación de nuevas variables como ser los días de cobertura del seguro (aplicando diferencia entre las fechas de inicio y fin de cobertura), asimismo, la edad se calculó a partir de la fecha de nacimiento y se realizó la conversión a variable numérica para proceder a aplicar la categorización por rangos de edades con la creación de una nueva variable.
...
INDICADOR 1: Volumen de ingresos por departamento
- Las regionales con mayores ingresos son Santa Cruz, La Paz y Cochabamba. Sucre, Beni, Oruro y Potosí y Pando alcanzaron la meta de ingresos por Seguros para dicho periodo.
- La meta a nivel nacional fue alcanzada en un 100% pese a los resultados del eje troncal.

INDICADOR 2: Concentración de volumen de créditos
Se resalta un alto riesgo en 20% de los principales clientes dado que representan un 69% de la cartera de ingreso por seguros.

INDICADOR 3: Se identifica que existe un equilibrio entre las regiones respecto a los resultados por ingresos, por lo que se cumple la meta requerida.

INDICADOR 4: Respecto a la evaluación de concentración por tipo de crédito se observa que se cumple lo requerido respecto a que los créditos PYME e Hipotecarios sean aquellos que tienen un límite promedio de Bs250.000

INDICADOR 5: Respecto a cada tipo de crédito se debe evaluar que el 75% de los créditos supere el máximo permitido en valor asegurado, identificando a 5 de los 18 tipo de créditos con incumplimiento.

INDICADOR 6: Debido al riesgo de tipo de cambio, el Banco pretende tener como máximo límite un 98% de las operaciones en moneda nacional, por lo que actualmente cumple con lo requerido para reducir el impacto de falta de dolares o devaluación de moneda.
 
INDICADOR 7:Respecto a las metas de ingresos por regionales se obtuvo el total de ingresos a una fecha corte donde se pudo evidenciarf que las regionales con cartera mas amplia, aun no cumplieron sus metas definidas, esto a razón que sus metas son mas amplias por el número de clientes afiliados

INDICADOR 8: El tipo de seguros Personas tiene un promedio de edad 5 años por debajo del promedio de edad del tipo de seguros generales, lo que nos sugiere que las personas de mayor edad cuentan con seguros generales(seguros de bienes, etc) por contar con un mayor capital de trabajo.

INDICADOR 9: Se puedo observar que según reglas de la empresa hay algunos acuerdos que no cumplieron con las tasas especificadas lo cual podria tener observaciones por el regulador. En general se puedde observar que a mayor valor asegurado, la tasa prima del seguro es menor. 
